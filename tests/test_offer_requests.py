...
def test_get_offer_request_by_id(requests_mock):
    with fixture('get-offer-request-by-id', '/offer_requests/id',
...
def test_get_offer_requests(requests_mock):
    ...
    end_pagination_url = 'http://someaddress/air/offer_requests?limit=50' + \
...
def test_create_offer_request(requests_mock):
    with fixture('create-offer-request', '/offer_requests?return_offers=false',
        ...
        slices = [{'departure_date': '2100-02-27',
                   'destination': 'LGW',
                   'origin': 'LIS'}]
        offer_request = client.offer_requests.create().\
            passengers(passengers).slices(slices).execute()
        assert offer_request.id == 'orq_00009hjdomFOCJyxHG7k7k'
